...
def check_plane():
	pointcloud = np.fromfile('/home/mano/work/robotics_ws/data/plane.bin', dtype=np.float32, count=-1).reshape([-1,3])
	x = pointcloud[:, 0]  
	y = pointcloud[:, 1]  
	z = pointcloud[:, 2]  
	...
	normals = np.fromfile('/home/mano/work/robotics_ws/data/plane_normals.bin', dtype=np.float32, count=-1).reshape([-1,3])
...
def check_sphere():
	pointcloud = np.fromfile('/home/mano/work/robotics_ws/data/sphere.bin', dtype=np.float32, count=-1).reshape([-1,3])
	x = pointcloud[:, 0]  
	y = pointcloud[:, 1]  
	z = pointcloud[:, 2]  
	...
	normals = np.fromfile('/home/mano/work/robotics_ws/data/sphere_normals.bin', dtype=np.float32, count=-1).reshape([-1,3])
	mayavi.mlab.quiver3d(x, y, z, normals[:,0], normals[:,1], normals[:,2], line_width=0.2, scale_factor=0.2, figure=fig)
